...
GironaStream::GironaStream(std::string serialportname, SerialPortStream::BaudRate baudrate):
		SerialPortStream(serialportname.c_str(), baudrate),
		pbmregex("^RECVPBM,(\\d+),(\\d+),(\\d+),(\\d+(?:\\.\\d+)?),(-(?:\\d+(?:\\.\\d+)?)),(\\d+(?:\\.\\d+)?),(\\d+(?:\\.\\d+)?)(,)"){
...
ICommsLink& GironaStream::operator >> (DataLinkFramePtr & dlf)
{
    ...
    while(!receivedPacket)
    {
        ...
        int notNameLength = ReadUntil((uint8_t*)notification, (uint8_t*) ":", 1, MAX_CMDNAME_LENGTH) - 1;
        char * notLengthPtr = notification + notNameLength + 1;

        int notLengthLength = ReadUntil((uint8_t*) notLengthPtr, (uint8_t*) ":", 1, MAX_NOTIFICATION_LENGHTFIELD_LENGTH) - 1;
        ...
        {
            throw CommsException(std::string("Expected integer")+std::string(e.what()), COMMS_EXCEPTION_UNKNOWN_ERROR);
        }
        ...
        if(notNameLength == 2 && *notification == 'A' && *(notification+1) == 'T')
        {
            //SIMPLE NOTIFICATION
