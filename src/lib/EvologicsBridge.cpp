...
EvologicsBridge::EvologicsBridge(ICommsDevice * _device, int _baudrate, DataLinkFrame::fcsType chksum):
...
void EvologicsBridge::SetLocalAddr(int _localAddr) {
...
void EvologicsBridge::_SendInitCommands()
{
	...
	*device << AT0 	  << endOfCmd  //Ensure data mode
			...
			<< ATZU0  << endOfCmd  //Disable USBLLONG (will take effect only in USBL modem
			<< ATZX0  << endOfCmd  //Disable extended notifications
			<< ATZC  + std::to_string(clusterSize) << endOfCmd  //Set cluster size (packets per packet train)
            << ATALX + std::to_string(localAddr)   << endOfCmd; //Set local address

    Utils::Sleep(2000);
    ClearTransmissionBuffer();
    Utils::Sleep(2000);
    *device	<< ATARX + std::to_string(remoteAddr)  << endOfCmd  //Set remote address
