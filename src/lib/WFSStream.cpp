...
void WFSStream::SetNotificationReceivedCallback(f_notification func)
{
    notificationReceivedCallback = func;
}

void WFSStream::_InitNotificationsFilter()
{
...
int WFSStream::ReadData(void * dbuf, int n, bool block)
{
	int reqn;
	if(ndata < n)
	{
	...
	int nreturn; //the number of valid bytes that will be returned (nreturn <= ret)
	...
	if(nreturn > 0)
	{
		...
		else if(firstNoValid > end) //there are valid bytes that will not be returned yet (n < ret)
		{
